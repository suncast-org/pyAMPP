...
def test_session_save_includes_template_flag():
    """Ensure session save writes the template-mode key to settings."""
    source = Path(__file__).resolve().parents[1] / "gxbox" / "gxampp.py"
    text = source.read_text(encoding="utf-8")
    assert 'self._settings.setValue("session/entry_is_template_only", self._entry_is_template_only)' in text
